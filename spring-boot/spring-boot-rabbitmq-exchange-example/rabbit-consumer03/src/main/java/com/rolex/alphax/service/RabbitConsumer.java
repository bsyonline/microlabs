...
import static com.rolex.alphax.config.RabbitConfig.*;
...
public class RabbitConsumer {
    ...
    public void receiveDirectMsg(String msg, Channel channel, Message message) {
        log.info("Consumer03-{}接收到来自{}队列的Direct消息：{}", Thread.currentThread().getName(), DIRECT_QUEUE, msg);
        try {
    ...
    public void receive(String msg, Channel channel, Message message) {
        log.info("Consumer03-{}接收到来自{}队列的Topic消息：{}", Thread.currentThread().getName(), TOPIC_QUEUE3, msg);
        try {
            channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
            log.info("消费成功，返回ACK");
        } catch (Exception e) {
    ...
    public void receiveFanoutMsg(String msg, Channel channel, Message message) {
        log.info("Consumer03-{}接收到来自{}队列的Fanout消息：{}", Thread.currentThread().getName(), FANOUT_QUEUE3, msg);
        try {
            channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
            log.info("消费成功，返回ACK");
        } catch (Exception e) {
    ...
    @RabbitListener(queues = HEADERS_QUEUE3)
    public void receiveHeadersMsg(String msg, Channel channel, Message message) {
        log.info("Consumer03-{}接收到来自{}队列的Headers消息：{}", Thread.currentThread().getName(), HEADERS_QUEUE3, msg);
        try {
            channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
            log.info("消费成功，返回ACK");
        } catch (Exception e) {
